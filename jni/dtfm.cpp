...
static unsigned int _dtfm_calc_list_chunk_size(wave_list_chunk_data* p_list_chunks,	int	list_chunk_cnt);
 ...
 *	@param	p_list_chunks	リストチャンクの配列
 *	@param	list_chunk_cnt	リストチャンク数
 *	
 */
void dtfm_wave_file_header(dtfm_chanel* p_chanel, 
		int data_size, 
		unsigned char* p_buff,
		wave_list_chunk_data* p_list_chunks,
		int	list_chunk_cnt)
{
	...
	// LISTチャンクのサイズを取得
	sizesize += _dtfm_calc_list_chunk_size(p_list_chunks, list_chunk_cnt);
	if( list_chunk_cnt != 0 ) {
		sizesize+= 8;	// 'LIST'	ChunkSize-8
	}
 ...
 *
 *	@retval	0	成功
 *	@retval	-1	ファイルをオープンできない。
 *	@retval	-2	ファイル書き込みに失敗
 */
int dtfm_update_wave_file_header_file(
		dtfm_chanel* p_chanel, 
		int data_size, 
		wave_list_chunk_data* p_list_chunks,
		int	list_chunk_cnt,
...
{
	int fh1;
	unsigned char head[WAVE_FILE_HEADER_SIZE];
	unsigned byteswritten;


	dtfm_wave_file_header(p_chanel, data_size, head, p_list_chunks, list_chunk_cnt);

	fh1 = _open(p_file_path, _O_WRONLY | _O_BINARY );
	if( fh1 == -1 ) {
		return -1;
	}
	if(( byteswritten = _write( fh1, head, sizeof( head ))) == -1 ) {
	...
	if( list_chunk_cnt != 0 ) {
		...
		// LISTチャンクサイズを取得
		chunk_size = _dtfm_calc_list_chunk_size(p_list_chunks, list_chunk_cnt);


		fh1 = _open(p_file_path, _O_WRONLY | _O_BINARY | O_APPEND );
		if( fh1 == -1 ) {
			return -1;
		}


		if(( byteswritten = _write( fh1, "LIST", 4)) == -1 ) {	// 'LIST'

		}
		u4_work = _dtfm_cnv_u4_native_2_intel_byte(chunk_size);
		if(( byteswritten = _write( fh1, &u4_work, 4)) == -1 ) {	// ChunkSize-8

		}
		if(( byteswritten = _write( fh1, "INFO", 4)) == -1 ) {	// 'INFO'

		}
		for( i = 0; i < list_chunk_cnt; i++) {
			if(( byteswritten = _write( fh1, p_list_chunks[i].name, 4)) == -1 ) {	// 'INFOタイプ'の名前

			}
			u4_work = _dtfm_cnv_u4_native_2_intel_byte(p_list_chunks[i].data_size);
			if(( byteswritten = _write( fh1, &u4_work, 4)) == -1 ) {	//	'INFOタイプ'のサイズ

			}
			if(( byteswritten = _write( fh1, p_list_chunks[i].data, 
					p_list_chunks[i].data_size)) == -1 ) {	// 'INFOタイプ'のデータ

			}
			if( (p_list_chunks[i].data_size % 2) != 0 ) {
				// 奇数の場合はさらにデータ00hを追加して偶数にする。
				u4_work = 0;
				if(( byteswritten = _write( fh1, &u4_work, 1)) == -1 ) {	//	'INFOタイプ'のサイズ
		...
		int	list_chunk_cnt)
{
	...
	if( list_chunk_cnt != 0 ) {
		...
		for( i = 0; i < list_chunk_cnt; i++) {
			fwrite(p_list_chunks[i].name, 4, 1, fp);

			u4_work = _dtfm_cnv_u4_native_2_intel_byte(p_list_chunks[i].data_size);
			fwrite(&u4_work, 4, 1, fp);

			fwrite(p_list_chunks[i].data, 
					p_list_chunks[i].data_size, 1, fp);
			if( (p_list_chunks[i].data_size % 2) != 0 ) {
				// 奇数の場合はさらにデータ00hを追加して偶数にする。
				u4_work = 0;
				fwrite(&u4_work, 1, 1, fp);
			}
		}
	}
...
static unsigned int _dtfm_calc_list_chunk_size(
			wave_list_chunk_data* p_list_chunks,
			int	list_chunk_cnt)
{
	unsigned int chunk_size = 0;
	...
	chunk_size = 4;	// 'INFO'
	for( i = 0; i < list_chunk_cnt; i++) {
		chunk_size += 4; // 'INFOタイプ'の名前
		chunk_size += 4; // 'INFOタイプ'のサイズ
		/*
			ここから実際の情報文字列(日本語も可)
			　可変長文字列+00h
		...
		*/
		chunk_size += p_list_chunks[i].data_size;
		if( (p_list_chunks[i].data_size % 2) != 0 ) {
			chunk_size += 1;
		}
	}

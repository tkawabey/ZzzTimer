...
public class ResponseHandler {
	...
	public static void buyPageIntentResponse(PendingIntent pendingIntent,
		...
		if (sPurchaseObserver == null) {
			if (MyLog.isDebugMod()) {
				MyLog.logt(TAG, "UI is not running");
			}
			return;
		}
	...
	public static void purchaseResponse(final Context context,
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				PurchaseAccessor acc = new PurchaseAccessor(context);
				int quantity = acc.updatePurchase(orderId, productId,
	 ...
	 *            request
	 */
	public static void responseCodeReceived(final Context context,
			final RequestPurchase request, final long requestId, final ResponseCode responseCode) {
		if (MyLog.isDebugMod()) {
			MyLog.logf(TAG, "responseCodeReceived start");
			MyLog.logf(TAG, "   ProductId   : " + request.mProductId);
			MyLog.logf(TAG, "   requestId   : " + requestId);
			MyLog.logf(TAG, "   responseCode: " + responseCode);
		}
		if (sPurchaseObserver != null) {
			sPurchaseObserver.onRequestPurchaseResponse(request, requestId, responseCode);
		}
		new Thread(new Runnable() {
			@Override
			public void run() {
				PurchaseAccessor acc = new PurchaseAccessor(context);
				int quantity = acc.updatePurchase("" + requestId, request.mProductId,
						PurchaseState.PURCHASED, new Date().getTime(), request.mDeveloperPayload);
			}
		}).start();
		if (MyLog.isDebugMod()) {
			MyLog.logf(TAG, "responseCodeReceived end");
		}
	}

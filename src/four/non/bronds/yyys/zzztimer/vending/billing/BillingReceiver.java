...
public class BillingReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (MyLog.isDebugMod()) {MyLog.logf(TAG, "onReceive start action :" + action);}
        if (Constant.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
            ...
            if (MyLog.isDebugMod()) {
            	...
            	MyLog.logf(TAG, "   responseCodeIndex  :" + responseCodeIndex);
            }
            checkResponseCode(context, requestId, responseCodeIndex);
        } else {
        	MyLog.logt(TAG, "unexpected action: " + action);
        }
        if (MyLog.isDebugMod()) {MyLog.logf(TAG, "onReceive end");}
    }
    ...
    private void purchaseStateChanged(Context context, String signedData, String signature) {
        Intent intent = new Intent(Constant.ACTION_PURCHASE_STATE_CHANGED);
        intent.setClass(context, BillingService.class);
        intent.putExtra(Constant.INAPP_SIGNED_DATA, signedData);
        intent.putExtra(Constant.INAPP_SIGNATURE, signature);
    ...
    private void notify(Context context, String notifyId) {
        Intent intent = new Intent(Constant.ACTION_GET_PURCHASE_INFORMATION);
        intent.setClass(context, BillingService.class);
    ...
    private void checkResponseCode(Context context, long requestId, int responseCodeIndex) {
        Intent intent = new Intent(Constant.ACTION_RESPONSE_CODE);
        intent.setClass(context, BillingService.class);
        intent.putExtra(Constant.INAPP_REQUEST_ID, requestId);
        intent.putExtra(Constant.INAPP_RESPONSE_CODE, responseCodeIndex);
        context.startService(intent);
    }
}
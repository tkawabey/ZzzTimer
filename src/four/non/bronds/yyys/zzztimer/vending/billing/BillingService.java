...
import four.non.bronds.yyys.zzztimer.vending.billing.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    abstract class BillingRequest {
        ...
        public boolean runRequest() {
        	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runRequest start");}
        	try {
	            if (runIfConnected()) {
	                return true;
	            }
        		...
        		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runRequest end");}
        	}
            return false;
        }
        ...
        public boolean runIfConnected() {
        	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runIfConnected start");}
        	try {
            if (mService != null) {
                try {
                    mRequestId = run();
                    if (MyLog.isDebugMod()) {
                    	MyLog.logt(TAG, "      request id: " + mRequestId);
                    }
                    if (mRequestId >= 0) {
        		...
        		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runIfConnected end");}
        	}
            return false;
        }
        ...
        protected void onRemoteException(RemoteException e) {
        	MyLog.logt(TAG, "remote billing service crashed");
        	MyLog.loge(TAG, e);
            mService = null;
        }
        ...
        protected Bundle makeRequestBundle(String method) {
            ...
            request.putString(Constant.BILLING_REQUEST_METHOD, method);
            request.putInt(Constant.BILLING_REQUEST_API_VERSION, 1);
            request.putString(Constant.BILLING_REQUEST_PACKAGE_NAME, getPackageName());
            return request;
        }

        protected void logResponseCode(String method, Bundle response) {
            ResponseCode responseCode = ResponseCode.valueOf(
                    response.getInt(Constant.BILLING_RESPONSE_RESPONSE_CODE));
            if (MyLog.isDebugMod()) {
            	MyLog.logt(TAG, method + " received " + responseCode.toString());
            }
        }
    }
    ...
    class CheckBillingSupported extends BillingRequest {
         ...
         * available.
         *
         * @pram itemType Either Constant.ITEM_TYPE_INAPP or Constant.ITEM_TYPE_SUBSCRIPTION, indicating
...
/*        public CheckBillingSupported(String itemType) {
        ...
        protected long run() throws RemoteException {
            ...
            if (mProductType != null) {
                request.putString(Constant.BILLING_REQUEST_ITEM_TYPE, mProductType);
            }
            Bundle response = mService.sendBillingRequest(request);
            int responseCode = response.getInt(Constant.BILLING_RESPONSE_RESPONSE_CODE);
            if (MyLog.isDebugMod()) {
            	MyLog.logi(TAG, "CheckBillingSupported response code: " +
    ...
    public class RequestPurchase extends BillingRequest {
         ...
         * @param itemId  The ID of the item to be purchased. Will be assumed to be a one-time
         *                purchase.
         * @param itemType  Either Constant.ITEM_TYPE_INAPP or Constant.ITEM_TYPE_SUBSCRIPTION,
        ...
        protected long run() throws RemoteException {
        	if (MyLog.isDebugMod()) {
        		MyLog.logf(TAG, getClass().getSimpleName() + "::run start");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       cmd               :" + "REQUEST_PURCHASE");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mProductId        :" + mProductId);
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mProductType      :" + mProductType);
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mDeveloperPayload :" + mDeveloperPayload);
        	}
        	try {
	            Bundle request = makeRequestBundle("REQUEST_PURCHASE");
	            request.putString(Constant.BILLING_REQUEST_ITEM_ID, mProductId);
	            if( mProductType != null ) {
	            	request.putString(Constant.BILLING_REQUEST_ITEM_TYPE, mProductType);
	            }
	            // Note that the developer payload is optional.
	            if (mDeveloperPayload != null) {
	                request.putString(Constant.BILLING_REQUEST_DEVELOPER_PAYLOAD, mDeveloperPayload);
	            }
	            Bundle response = mService.sendBillingRequest(request);
	            PendingIntent pendingIntent
	                    = response.getParcelable(Constant.BILLING_RESPONSE_PURCHASE_INTENT);
	            if (pendingIntent == null) {
	            	MyLog.logi(TAG, "Error with requestPurchase");
	                return Constant.BILLING_RESPONSE_INVALID_REQUEST_ID;
	            }
	
	            Intent intent = new Intent();
	            ResponseHandler.buyPageIntentResponse(pendingIntent, intent);
	            return response.getLong(Constant.BILLING_RESPONSE_REQUEST_ID,
	                    Constant.BILLING_RESPONSE_INVALID_REQUEST_ID);
        	} finally {
        		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::run end");}
        	}
        }

        @Override
        protected void responseCodeReceived(long requestId, ResponseCode responseCode) {
        	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::responseCodeReceived start :" + responseCode + "   requestId:" + requestId);}
        	try {
        		ResponseHandler.responseCodeReceived(BillingService.this, this, requestId, responseCode);
        	} finally {
        		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::responseCodeReceived end");}
        	}
        }
    }
    ...
    class ConfirmNotifications extends BillingRequest {
        ...
        protected long run() throws RemoteException {
        	if (MyLog.isDebugMod()) {
        		MyLog.logf(TAG, getClass().getSimpleName() + "::run start");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       cmd          :" + "CONFIRM_NOTIFICATIONS");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mNotifyIds   :" + mNotifyIds);
        	}
        	try {
	            Bundle request = makeRequestBundle("CONFIRM_NOTIFICATIONS");
	            request.putStringArray(Constant.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
	            Bundle response = mService.sendBillingRequest(request);
    ...
    class GetPurchaseInformation extends BillingRequest {
        ...
        protected long run() throws RemoteException {
        	if (MyLog.isDebugMod()) {
        		MyLog.logf(TAG, getClass().getSimpleName() + "::run start");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       cmd          :" + "GET_PURCHASE_INFORMATION");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mNonce       :" + mNonce);
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mNotifyIds   :" + mNotifyIds);
        	}
        	try {
	            ...
	            request.putLong(Constant.BILLING_REQUEST_NONCE, mNonce);
	            request.putStringArray(Constant.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
	            Bundle response = mService.sendBillingRequest(request);
    ...
    public class RestoreTransactions extends BillingRequest {
        ...
        protected long run() throws RemoteException {
        	if (MyLog.isDebugMod()) {
        		MyLog.logf(TAG, getClass().getSimpleName() + "::run start");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       cmd          :" + "RESTORE_TRANSACTIONS");
        		MyLog.logf(TAG, getClass().getSimpleName() + "       mNonce       :" + mNonce);
        	}
        	try {
	            mNonce = Security.generateNonce();
        ...
        @Override
        protected void onRemoteException(RemoteException e) {
        	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::onRemoteException start");}
        	try {
        		super.onRemoteException(e);
        		Security.removeNonce(mNonce);
        	} finally {
        		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::onRemoteException end");}
        	}
        }

        @Override
        protected void responseCodeReceived(long requestId, ResponseCode responseCode) {
        	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::responseCodeReceived start");}
        	try {
    ...
    public void handleCommand(Intent intent, int startId) {
    	...
    	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::handleCommand start  action:" + action);}
    	try {
	        if (Constant.ACTION_CONFIRM_NOTIFICATION.equals(action)) {
	            String[] notifyIds = intent.getStringArrayExtra(Constant.NOTIFICATION_ID);
	            confirmNotifications(startId, notifyIds);
	        } else if (Constant.ACTION_GET_PURCHASE_INFORMATION.equals(action)) {
	            String notifyId = intent.getStringExtra(Constant.NOTIFICATION_ID);
	            getPurchaseInformation(startId, new String[] { notifyId });
	        } else if (Constant.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
	            String signedData = intent.getStringExtra(Constant.INAPP_SIGNED_DATA);
	            String signature = intent.getStringExtra(Constant.INAPP_SIGNATURE);
	            purchaseStateChanged(startId, signedData, signature);
	        } else if (Constant.ACTION_RESPONSE_CODE.equals(action)) {
	            long requestId = intent.getLongExtra(Constant.INAPP_REQUEST_ID, -1);
	            int responseCodeIndex = intent.getIntExtra(Constant.INAPP_RESPONSE_CODE,
	                    ResponseCode.RESULT_ERROR.ordinal());
	            ResponseCode responseCode = ResponseCode.valueOf(responseCodeIndex);
	            checkResponseCode(requestId, responseCode);
	        }
    	} finally {
    		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::handleCommand end");}
    	}
    }
    ...
    private boolean bindToMarketBillingService() {
        try {
        	if (MyLog.isDebugMod()) {
        		MyLog.logt(TAG, "binding to Market billing service");
            }
            boolean bindResult = bindService(
                    new Intent(Constant.MARKET_BILLING_SERVICE_ACTION),
            ...
            if (bindResult) {
            	...
            	MyLog.logi(TAG, "Could not bind to service.");
            }
     ...
     * @pram itemType Either Constant.ITEM_TYPE_INAPP or Constant.ITEM_TYPE_SUBSCRIPTION, indicating the
...
*/
     ...
     * receives an intent with the action {@link Constant#ACTION_NOTIFY}.
     ...
     * @param itemType  Either Constant.ITEM_TYPE_INAPP or Constant.ITEM_TYPE_SUBSCRIPTION, indicating
     ...
     * in an intent with the action {@link Constant#ACTION_PURCHASE_STATE_CHANGED}.
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
    	if (MyLog.isDebugMod()) {
    		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::purchaseStateChanged start");}
    		MyLog.logf(TAG, getClass().getSimpleName() + "    startId       :" + startId);
    		MyLog.logf(TAG, getClass().getSimpleName() + "    signedData    :" + signedData);
    		MyLog.logf(TAG, getClass().getSimpleName() + "    signature     :" + signature);
    	}
    	try {
    		...
    		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::purchaseStateChanged end");}
    	}
    }
    ...
    private void checkResponseCode(long requestId, ResponseCode responseCode) {
    	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::checkResponseCode start");}
    	try {
	        BillingRequest request = mSentRequests.get(requestId);
	        if (request != null) {
	        	if (MyLog.isDebugMod()) {
	        		MyLog.logt(TAG, request.getClass().getSimpleName() + ": " + responseCode);
	            }
	            request.responseCodeReceived(requestId, responseCode);
	        }
	        mSentRequests.remove(requestId);
    	} finally {
    		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::checkResponseCode end");}
    	}
    }
    ...
    private void runPendingRequests() {
    	if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runPendingRequests start");}
    	try {
	        int maxStartId = -1;
	        ...
	        if (maxStartId >= 0) {
	        	if (MyLog.isDebugMod()) {
	        		MyLog.logi(TAG, "stopping service, startId: " + maxStartId);
	            }
	            stopSelf(maxStartId);
	        }
    	} finally {
    		if (MyLog.isDebugMod()) {MyLog.logf(TAG, getClass().getSimpleName() + "::runPendingRequests end");}
    	}
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
    	if (MyLog.isDebugMod()) {
    		MyLog.logi(TAG, "Billing service connected");
        }
    ...
    public void onServiceDisconnected(ComponentName name) {
    	MyLog.logi(TAG, "Billing service disconnected");
        mService = null;
    }
